...
def save_restaurant_data_to_csv(data, output_file) -> None:
    """
    Save the extracted restaurant data to a CSV file.
    
    param data: list of dictionaries with the extracted data
    
    param output_file: string with the path to the output CSV file
    """
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile: # open the CSV file
        writer = csv.DictWriter(csvfile, fieldnames=csv_headers) # create a CSV DictWriter object
...
def forming_a_csv():
    ...
    csv_output_file = 'michelin_restaurant_data.csv' # Output CSV file to save the extracted data
    ...
    save_restaurant_data_to_csv(all_restaurant_data, csv_output_file) # save the extracted data to a CSV file
